...
bool AESDecryptContext::init(const Crypto::Key& key,
                             const Crypto::IV&  iv,
                             bool               pkcs7)
{
    ...
    if (psa_cipher_decrypt_setup(&mOperation, mKeyId, algorithm) != PSA_SUCCESS)
    {
        psa_destroy_key(mKeyId);
        return false;
    }
...
bool AESDecryptContext::update(const uint8_t* in,
                               std::size_t    inLen,
                               uint8_t*       out,
...
bool AESDecryptContext::finish(uint8_t*     out,
                               std::size_t  maxOut,
                               std::size_t& outLen)
{
    if (mState != State::kInitialized)
