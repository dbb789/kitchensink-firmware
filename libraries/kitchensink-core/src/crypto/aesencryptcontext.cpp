...
bool AESEncryptContext::init(const Crypto::Key& key, const Crypto::IV& iv)
{
    ...
    if (psa_cipher_encrypt_setup(&mOperation, mKeyId, PSA_ALG_CBC_NO_PADDING) != PSA_SUCCESS)
    {
        psa_destroy_key(mKeyId);
        return false;
    }
...
bool AESEncryptContext::update(const uint8_t* in, uint8_t* out, std::size_t size)
{
